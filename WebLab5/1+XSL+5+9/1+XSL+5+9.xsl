...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <html>
            <head><title>XSL其它元素应用示例</title></head>
            <body>
                ...
                <p>Titles:
                    ...
                    <xsl:element name="img">
                        <xsl:attribute name="src">ballet3.png</xsl:attribute>
                        <xsl:attribute name="alt">This is a picture</xsl:attribute>
                    </xsl:element>
                    <p>Total Price = <xsl:value-of select="sum(CATALOG/CD/PRICE)"/>
                    </p>
                    <p><xsl:value-of select="substring(CATALOG/CD/TITLE, 1, 3)"/>
                    </p>
                </p>
            </body>
        </html>
